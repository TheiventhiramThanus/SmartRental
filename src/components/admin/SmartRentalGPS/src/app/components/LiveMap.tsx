...
export function LiveMap({ currentPosition, pathPoints, isTracking, mapStyle = 'default', onMapStyleChange }: LiveMapProps) {
  ...
  useEffect(() => {
    ...
    const getTileLayerUrl = (style: string) => {
      switch (style) {
        ...
        default:
          // Google Maps-style tiles (using Google's satellite/roadmap hybrid)
  ...
  useEffect(() => {
    ...
    if (!markerRef.current) {
      ...
      if (prevPos && isTracking) {
        // Only animate if position changed significantly (more than ~1 meter)
        ...
        if (distance > 1) {
          ...
          // Smooth pan map to follow (if needed)
  ...
  useEffect(() => {
    ...
    if (!polylineRef.current) {
      // Create new polyline with enhanced visibility
      polylineRef.current = L.polyline(latLngs, {
        color: '#2563EB', // Darker blue for better visibility
