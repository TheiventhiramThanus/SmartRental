...
import { signUp, updateDocument, signInWithGoogle } from '../../firebase';
...
export function RegisterPage({ onRegister, onNavigate }: RegisterPageProps) {
  ...
  const validateForm = () => {
    ...
    if (!phoneRegex.test(formData.phone) || formData.phone.length < 9) return "Invalid phone number";
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const firebaseUser = await signUp(formData.email, formData.password, formData.name);

      // Add user details to Firestore 'users' collection
      let role = 'customer';
      if (formData.email === 'admin@demo.com') role = 'admin';
      else if (formData.email === 'staff@demo.com') role = 'staff';
  ...
  const handleGoogleRegister = async () => {
    try {
      ...
      setError(err.message || 'Error continuing with Google');
    } finally {
      setIsLoading(false);
    }
  };
